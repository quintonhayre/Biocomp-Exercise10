#Quinton Hayre Ex10 
library(ggplot2)
setwd("/Users/Quintonhayre/Desktop/R_Biocomp/Biocomp-Exercise10/")
#1
#Find some data on two variables that you would expect to be related to each other. These data can come from your own research, 
#your daily life, or the internet. Enter those data into a text file or Excel and then save a text file, and write a 
#script that loads this text file and produces a scatter plot of those two variables that includes a trend line.

#Data: https://www.basketball-reference.com/players/j/jamesle01.html
...
#Lebron james minutes played vs points scores: deviated from linear trend I expected 
lebron = read.table("lebron.txt", sep = "\t", header = T)
ggplot(lebron, aes(x= MP, y= PTS)) + geom_point() + stat_smooth(method = "lm") + 
  xlab("Average Minutes Played per Season") + ylab("Average Points per Season") + 
  labs(title = "Lebron James", subtitle = "Seasons: 2003 - 2019") + theme_classic()
...
#You may want to “jitter” the points (geom_jitter()) to make it easier to see all of the observations within a population 
#in your scatter plot. Alternatively, you could also try setting the alpha argument in geom_scatterplot() to 0.1. 
...
ggplot(data, aes(x = region, y = observations))  + stat_summary(fun = mean, geom = "bar")
  #check
tapply(data$observations, data$region, mean)

#Scatter 
ggplot(data, aes(x = region, y = observations))  + geom_jitter()

#Why 
#We can see discrepency in concentration in the South region where the average tells a story of a region where almost no observations occur
  #since there are two high density spots at either side of the range 
#As well we can see the ones with more density that an average would tell a better story of such as North
#Meanwhile east and west are fairly well represented by an average, however they have a lot more standard variation than North 
  #which is not told in a mean graph 
